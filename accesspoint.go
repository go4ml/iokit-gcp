...
func Lookup(gsurl string) (ap *AccessPoint, loc Location, err error) {
	...
	if len(u.Host) == 0 {
		err = errstr.New("bad gsurl (empty hostname) `" + gsurl + "`")
		return
	}
	if u.Host[0] != '$' {
		err = errstr.New("bad gsurl (hostname must start with '$') `" + gsurl + "`")
		return
	}
	mu.Lock()
	defer mu.Unlock()
	...
	if ap == nil {
		if ep == "" {
			...
			if ap == nil {
				err = errstr.New("not found access point for gsurl `" + gsurl + "`")
				return
			}
		}
		registry[ep] = ap
	...
	if ap.Bucket != "" {
		...
		if j < 0 {
			err = errstr.New("bad gsurl (there is no key name) `" + gsurl + "`")
		}
...
func defineAccessPoint(gsdef string) (ap *AccessPoint, err error) {
	if strings.HasPrefix(gsdef, "json://") {
		...
		if len(s) != 4 {
			return nil, errstr.New("invalid access pointdefinition (bad cound of elements)")
		}
		var f *os.File
		var dat []byte
		f, err = os.Open(s[3])
		if err != nil {
			return nil, errstr.Wrap(err, "can't open credentials file")
		}
		dat, err = ioutil.ReadAll(f)
		if err != nil {
			return nil, errstr.Wrap(err, "failed to read credentials file")
		}
		if s[2] != "" {
			// encrypted
			dat, err = fu.Decrypt(s[2], dat)
			if err != nil {
				return nil, errstr.Wrap(err, "failed to decrypt credentials file")
			}
		}
		if err != nil {
			return nil, errstr.Wrap(err, "failed to read credentials file")
	...
	return nil, errstr.New("invalid access pointdefinition (no json:// prefix)")
}
